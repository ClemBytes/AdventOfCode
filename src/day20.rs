...
fn find_lowest_house_number_part2(n: u32) -> u32 {
    let max_houses = 1_000_000;
    let mut presents = vec![0u32; max_houses + 1];
    // presents[k] contains the number of presents delivered in house k
    for elf in 1..=max_houses {
        for i in 1..=50 {
            if i * elf <= max_houses {
                presents[i * elf] += 11 * elf as u32;
            } else {
                break;
            }
        }
    }
    for (i, &nb_presents) in presents.iter().enumerate() {
        if nb_presents >= n {
            return i as u32;
        }
    }
...
fn day20_part2(input: u32) {
    // Solve puzzle
    let res = find_lowest_house_number_part2(input);
    println!("Result part 2: {res}");
    assert_eq!(res, 705600);
